...
var _scrollbar_drag_offset := 0.0  # Y offset from thumb top when drag started
...
const SCROLLBAR_MARGIN := 6.0   # gap between scrollbar and content
...
func _ready() -> void:
	...
	add_child(_canvas)
	# Clipped container for scrollable content
	_clip_container = Control.new()
	_clip_container.name = "ClipContainer"
	_clip_container.clip_contents = true
	_clip_container.mouse_filter = Control.MOUSE_FILTER_IGNORE
	add_child(_clip_container)
	_content_canvas = Control.new()
	_content_canvas.name = "ContentCanvas"
	_content_canvas.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_content_canvas.connect("draw", _draw_content)
	_clip_container.add_child(_content_canvas)
	_setup_mic_capture()
...
func _scrollbar_gutter() -> float:
	## Extra right padding when content is scrollable
	if _max_scroll > 0.01:
...
func _mic_item_rect(index: int) -> Rect2:
	...
	var sy := _content_y_to_screen(cy)
	return Rect2(pr.position.x + PADDING + VU_MARGIN, sy, PANEL_WIDTH - PADDING * 2 - VU_MARGIN - _scrollbar_gutter(), ITEM_HEIGHT - 4)
...
func _api_input_rect() -> Rect2:
	...
	var sy := _content_y_to_screen(cy)
	return Rect2(pr.position.x + PADDING, sy, PANEL_WIDTH - PADDING * 2 - 60 - _scrollbar_gutter(), 28)

func _api_btn_rect() -> Rect2:
	var pr := _panel_rect()
	var cy := _api_section_content_y() + SECTION_HEADER_H + 4
	var sy := _content_y_to_screen(cy)
	return Rect2(pr.position.x + PANEL_WIDTH - PADDING - 50 - _scrollbar_gutter(), sy, 50, 28)
...
func _volume_slider_rect() -> Rect2:
	...
	var sy := _content_y_to_screen(cy)
	return Rect2(pr.position.x + PADDING + 30, sy, PANEL_WIDTH - PADDING * 2 - 70 - _scrollbar_gutter(), VOLUME_SLIDER_H)
...
func _process(delta: float) -> void:
	...
	# Update clip container to match content area and redraw content
	if _progress > 0.01:
		var ca := _content_area_rect()
		_clip_container.position = ca.position
		_clip_container.size = ca.size
		_content_canvas.position = Vector2(-ca.position.x, -ca.position.y)
		var vp_size := get_viewport().get_visible_rect().size
		_content_canvas.size = vp_size
	_content_canvas.queue_redraw()
...
func _update_hover() -> void:
	...
	if _max_scroll > 0.01:
		var ca := _content_area_rect()
		var sb_x := ca.position.x + ca.size.x - SCROLLBAR_WIDTH - 4
		var sb_hover_rect := Rect2(sb_x - 4, ca.position.y, SCROLLBAR_WIDTH + 8, ca.size.y)
		if sb_hover_rect.has_point(mouse):
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		var sb_mouse := _canvas.get_local_mouse_position()
		if event.pressed and _max_scroll > 0.01:
			var ca := _content_area_rect()
			var sb_x := ca.position.x + ca.size.x - SCROLLBAR_WIDTH - 4
			var sb_track_y := ca.position.y + 6
			var sb_track_h := ca.size.y - 12
			var sb_hit := Rect2(sb_x - 4, sb_track_y, SCROLLBAR_WIDTH + 8, sb_track_h)
			if sb_hit.has_point(sb_mouse):
				# Compute current thumb position
				var content_h := _content_height()
				var visible_h := _visible_height() - TITLE_H
				var thumb_ratio := clampf(visible_h / content_h, 0.15, 1.0)
				var thumb_h := maxf(sb_track_h * thumb_ratio, 30.0)
				var scroll_ratio := _scroll_offset / maxf(_max_scroll, 0.01)
				var thumb_y := sb_track_y + scroll_ratio * (sb_track_h - thumb_h)
				# Check if clicking on the thumb vs the track
				if sb_mouse.y >= thumb_y and sb_mouse.y <= thumb_y + thumb_h:
					# Grabbed the thumb — remember offset
					_scrollbar_dragging = true
					_scrollbar_drag_offset = sb_mouse.y - thumb_y
				else:
					# Clicked on track — jump to that position
					_scrollbar_dragging = true
					_scrollbar_drag_offset = thumb_h * 0.5
					var new_thumb_y := sb_mouse.y - _scrollbar_drag_offset
					var new_ratio := clampf((new_thumb_y - sb_track_y) / maxf(sb_track_h - thumb_h, 1.0), 0.0, 1.0)
					_scroll_target = new_ratio * _max_scroll
	...
	if event is InputEventMouseMotion and _scrollbar_dragging:
		...
		var thumb_ratio := clampf(visible_h / content_h, 0.15, 1.0)
		var thumb_h := maxf(sb_track_h * thumb_ratio, 30.0)
		var new_thumb_y := drag_mouse.y - _scrollbar_drag_offset
		var new_ratio := clampf((new_thumb_y - sb_track_y) / maxf(sb_track_h - thumb_h, 1.0), 0.0, 1.0)
		_scroll_target = new_ratio * _max_scroll
		_scroll_offset = _scroll_target  # Instant follow during drag
		get_viewport().set_input_as_handled()
		return
...
func _draw_panel() -> void:
	...
	# ── Scrollbar (drawn on top canvas, not clipped) ──
	if _max_scroll > 0.01:
...
func _draw_content() -> void:
	...
	cc.draw_string(font, Vector2(pr.position.x + PADDING, mic_sy + 20),
	...
	# Separator above
	cc.draw_line(Vector2(pr.position.x + PADDING, api_sy - 2),
		Vector2(pr.position.x + PANEL_WIDTH - PADDING, api_sy - 2),
		Color(0.3, 0.5, 0.8, 0.2 * alpha), 1.0)

	cc.draw_string(font, Vector2(pr.position.x + PADDING, api_sy + 20),
	...
	cc.draw_rect(input_r, input_bg, true)
	cc.draw_rect(input_r, input_border, false, 1.0)
	...
	cc.draw_string(font, Vector2(input_r.position.x + 8, input_r.position.y + input_r.size.y * 0.7),
	...
	cc.draw_rect(btn_r, btn_bg, true)
	cc.draw_rect(btn_r, btn_border, false, 1.0)
	cc.draw_string(font, Vector2(btn_r.position.x + 6, btn_r.position.y + btn_r.size.y * 0.7),
	...
	if _api_key_editing:
		...
		cc.draw_circle(Vector2(dot_x, dot_y), 5, Color(0.3, 0.5, 0.8, 0.5 * alpha))
		cc.draw_string(font, Vector2(dot_x - 3, dot_y + 4), "✏", HORIZONTAL_ALIGNMENT_LEFT, -1, 8, Color(0.7, 0.85, 1.0, alpha))
		...
		cc.draw_circle(Vector2(dot_x, dot_y), 5, Color(0.4, 0.6, 1.0, spin_alpha * alpha))
		cc.draw_circle(Vector2(dot_x, dot_y), 3, Color(0.2, 0.3, 0.5, 0.8 * alpha))
	elif _api_key_valid == 1:
		status_text = "   Clé valide"
		status_color = Color(0.3, 0.9, 0.4, 0.8 * alpha)
		cc.draw_circle(Vector2(dot_x, dot_y), 6, Color(0.15, 0.5, 0.2, 0.8 * alpha))
		cc.draw_circle(Vector2(dot_x, dot_y), 5, Color(0.2, 0.85, 0.35, 0.9 * alpha))
		cc.draw_string(font, Vector2(dot_x - 4, dot_y + 4), "✓", HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(1, 1, 1, alpha))
	elif _api_key_valid == 0:
		status_text = "   Clé invalide"
		status_color = Color(0.95, 0.3, 0.25, 0.8 * alpha)
		cc.draw_circle(Vector2(dot_x, dot_y), 6, Color(0.5, 0.1, 0.1, 0.8 * alpha))
		cc.draw_circle(Vector2(dot_x, dot_y), 5, Color(0.9, 0.2, 0.15, 0.9 * alpha))
		cc.draw_string(font, Vector2(dot_x - 3, dot_y + 4), "✗", HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(1, 1, 1, alpha))
	else:
		status_text = "   Clé requise pour démarrer"
		status_color = Color(0.9, 0.5, 0.3, 0.7 * alpha)
		cc.draw_circle(Vector2(dot_x, dot_y), 5, Color(0.6, 0.35, 0.15, 0.6 * alpha))
		cc.draw_string(font, Vector2(dot_x - 3, dot_y + 4), "!", HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(1, 0.8, 0.4, alpha))
	
	cc.draw_string(font, Vector2(pr.position.x + PADDING, status_sy),
	...
	# Separator above
	cc.draw_line(Vector2(pr.position.x + PADDING, lang_sy - 2),
		Vector2(pr.position.x + PANEL_WIDTH - PADDING, lang_sy - 2),
		Color(0.3, 0.5, 0.8, 0.2 * alpha), 1.0)

	cc.draw_string(font, Vector2(pr.position.x + PADDING, lang_sy + 20),
	...
	cc.draw_rect(sel_r, sel_bg, true)
	cc.draw_rect(sel_r, sel_border, false, 1.0)
	cc.draw_string(font, Vector2(sel_r.position.x + 10, sel_r.position.y + sel_r.size.y * 0.7),
	...
	cc.draw_string(font, Vector2(sel_r.position.x + sel_r.size.x - 18, sel_r.position.y + sel_r.size.y * 0.7),
	...
	if _lang_dropdown_open:
		for di in LANGUAGES.size():
			...
			cc.draw_rect(dir, item_bg, true)
			cc.draw_rect(dir, Color(0.2, 0.35, 0.6, 0.3 * alpha), false, 1.0)
			var item_label = LANGUAGES[di]["label"]
			if LANGUAGES[di]["code"] == _language:
				item_label = item_label + "  >"
			cc.draw_string(font, Vector2(dir.position.x + 10, dir.position.y + dir.size.y * 0.7),
	...
	# Separator above
	cc.draw_line(Vector2(pr.position.x + PADDING, vol_sy - 2),
		Vector2(pr.position.x + PANEL_WIDTH - PADDING, vol_sy - 2),
		Color(0.3, 0.5, 0.8, 0.2 * alpha), 1.0)

	cc.draw_string(font, Vector2(pr.position.x + PADDING, vol_sy + 20),
	...
	cc.draw_string(font, Vector2(icon_x, icon_y),
		icon_text, HORIZONTAL_ALIGNMENT_LEFT, 20, 12, Color(0.6, 0.75, 0.9, alpha))

	# Track background
	var track_color := Color(0.12, 0.12, 0.2, 0.8 * alpha)
	cc.draw_rect(Rect2(vol_sr.position.x, vol_sr.position.y, vol_sr.size.x, vol_sr.size.y), track_color, true)
	cc.draw_rect(Rect2(vol_sr.position.x, vol_sr.position.y, vol_sr.size.x, vol_sr.size.y),
	...
	if fill_w > 1.0:
		...
		cc.draw_rect(Rect2(vol_sr.position.x, vol_sr.position.y, fill_w, vol_sr.size.y), fill_color, true)
	...
	cc.draw_circle(thumb_c, thumb_r + 1, thumb_border)
	cc.draw_circle(thumb_c, thumb_r, thumb_color)
	...
	cc.draw_string(font, Vector2(pct_x, pct_y),
	...
	# Separator above
	cc.draw_line(Vector2(pr.position.x + PADDING, usage_sy - 2),
		Vector2(pr.position.x + PANEL_WIDTH - PADDING, usage_sy - 2),
		Color(0.3, 0.5, 0.8, 0.2 * alpha), 1.0)

	cc.draw_string(font, Vector2(pr.position.x + PADDING, usage_sy + 20),
	...
	cc.draw_string(font, Vector2(col1_x, r0_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col1_x + val_offset, r0_sy + row_h * 0.7),
		conn_val, HORIZONTAL_ALIGNMENT_LEFT, 60, icon_size, value_color)
	cc.draw_string(font, Vector2(col2_x, r0_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col2_x + val2_offset, r0_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col1_x, r1_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col1_x + val_offset, r1_sy + row_h * 0.7),
		pulses_val, HORIZONTAL_ALIGNMENT_LEFT, 60, icon_size, value_color)
	cc.draw_string(font, Vector2(col2_x, r1_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col2_x + val2_offset, r1_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col1_x, r2_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col1_x + val_offset, r2_sy + row_h * 0.7),
		audio_sent, HORIZONTAL_ALIGNMENT_LEFT, 60, icon_size, value_color)
	cc.draw_string(font, Vector2(col2_x, r2_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col2_x + val2_offset, r2_sy + row_h * 0.7),
	...
	cc.draw_string(font, Vector2(col1_x, note_sy + 8),
...
func _draw_scrollbar(alpha: float, ca: Rect2) -> void:
	var bar_x := ca.position.x + ca.size.x - SCROLLBAR_WIDTH - 4
	var track_h := ca.size.y - 12
	var track_y := ca.position.y + 6

	# Track background — visible rounded rail
	_canvas.draw_rect(Rect2(bar_x - 1, track_y - 1, SCROLLBAR_WIDTH + 2, track_h + 2),
		Color(0.08, 0.08, 0.15, 0.7 * alpha), true)
	_canvas.draw_rect(Rect2(bar_x - 1, track_y - 1, SCROLLBAR_WIDTH + 2, track_h + 2),
		Color(0.2, 0.35, 0.6, 0.3 * alpha), false, 1.0)

	# Thumb
	var content_h := _content_height()
	var visible_h := _visible_height() - TITLE_H
	var thumb_ratio := clampf(visible_h / content_h, 0.15, 1.0)
	var thumb_h := maxf(track_h * thumb_ratio, 30.0)
	var scroll_ratio := _scroll_offset / maxf(_max_scroll, 0.01)
	var thumb_y := track_y + scroll_ratio * (track_h - thumb_h)

	# Outer glow
	_canvas.draw_rect(Rect2(bar_x - 2, thumb_y - 1, SCROLLBAR_WIDTH + 4, thumb_h + 2),
		Color(0.3, 0.55, 1.0, 0.15 * alpha), true)
	# Thumb body — bright blue
	var thumb_color := Color(0.35, 0.6, 1.0, 0.85 * alpha)
	if _scrollbar_hovered:
		thumb_color = Color(0.5, 0.75, 1.0, 0.95 * alpha)
	_canvas.draw_rect(Rect2(bar_x, thumb_y, SCROLLBAR_WIDTH, thumb_h),
		thumb_color, true)
	# Thumb border
	_canvas.draw_rect(Rect2(bar_x, thumb_y, SCROLLBAR_WIDTH, thumb_h),
		Color(0.5, 0.75, 1.0, 0.5 * alpha), false, 1.0)
	# Center grip lines (visual indicator)
	var grip_cx := bar_x + SCROLLBAR_WIDTH * 0.5
	var grip_cy := thumb_y + thumb_h * 0.5
	for gi in 3:
		var gy := grip_cy + (gi - 1) * 4.0
		_canvas.draw_line(Vector2(bar_x + 2, gy), Vector2(bar_x + SCROLLBAR_WIDTH - 2, gy),
			Color(1, 1, 1, 0.3 * alpha), 1.0)


func _draw_vu_meter(alpha: float) -> void:
	var pr := _panel_rect()
	var cc := _content_canvas
	if _mics.is_empty():
		return
	...
	# Background
	cc.draw_rect(Rect2(vu_x_pos - 3, vu_sy_start - 3, VU_WIDTH + 6, vu_h_total + 6),
	...
	for s in range(num_segments):
		...
		cc.draw_rect(Rect2(vu_x_pos, sy, VU_WIDTH, segment_h), c, true)
...
func _draw_mic_item(index: int, alpha: float) -> void:
	...
	if is_selected:
		cc.draw_rect(rect, Color(0.15, 0.45, 0.25, 0.7 * alpha), true)
		cc.draw_rect(Rect2(rect.position.x, rect.position.y, 3, rect.size.y), Color(0.3, 1.0, 0.5, alpha), true)
		cc.draw_rect(rect, Color(0.3, 1.0, 0.5, 0.5 * alpha), false, 1.0)
	elif is_hovered:
		cc.draw_rect(rect, Color(0.2, 0.35, 0.6, 0.4 * alpha), true)
		cc.draw_rect(rect, Color(0.4, 0.7, 1.0, 0.4 * alpha), false, 1.0)

	var ix := rect.position.x + 12
	var iy := rect.position.y + rect.size.y * 0.5
	if is_selected:
		cc.draw_circle(Vector2(ix, iy), 7, Color(0.3, 1.0, 0.5, alpha))
		cc.draw_string(font, Vector2(ix - 5, iy + 5), "✓", HORIZONTAL_ALIGNMENT_LEFT, -1, 12, Color(0, 0, 0, alpha))
	else:
		cc.draw_circle(Vector2(ix, iy), 6, Color(0.4, 0.5, 0.6, 0.3 * alpha))
		cc.draw_circle(Vector2(ix, iy), 4, Color(0.06, 0.06, 0.12, 0.9 * alpha))
	...
	cc.draw_string(font, Vector2(ix + 16, rect.position.y + rect.size.y * 0.65),
	...
	if is_selected:
		...
		var by := iy - bs.y * 0.3
		cc.draw_rect(Rect2(bx - 4, by - 2, bs.x + 8, bs.y + 4), Color(0.2, 0.8, 0.4, 0.3 * alpha), true)
		cc.draw_string(font, Vector2(bx, by + bs.y - 1), "ACTIF", HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(0.4, 1.0, 0.6, alpha))
