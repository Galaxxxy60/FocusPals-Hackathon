...
def get_mood_context(lang: str = "en") -> str:
    ...
    Combines mood_bias AND confidence for a complete emotional picture.
    ...
    if lang == "en":
        ...
        if C < 0.3:
            mood += " You DON'T trust him at all right now — he's been switching back and forth to dodge your surveillance. You're hyper-vigilant. If he goes back to slacking, call it out directly: 'tu crois que j'ai pas vu ton manège ?' style."
        elif C < 0.6:
            mood += " You're suspicious of his sincerity — he's done some quick switches recently. Watch him closely."
        elif C >= 0.95:
        ...
        # Overlay confidence context
        if C < 0.3:
            mood += " Tu ne lui fais PLUS confiance — il a fait des allers-retours rapides pour esquiver ta surveillance. Tu es hyper-vigilante. S'il replonge, dis-le direct : genre 'tu crois que j'ai pas vu ton petit manège ?' ou 'encore ? sérieusement ?'."
        elif C < 0.6:
            mood += " Tu doutes de sa sincérité — il a fait des switches rapides récemment. Surveille-le de près."
        elif C >= 0.95:
            mood += " Tu lui fais confiance en ce moment. Il bosse régulièrement."
